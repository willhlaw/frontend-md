...
function render_code_folder_structure(folder, title, filters, callback) {
  ...
  var files = [];
  var excludeFolders = filters || [];

  if (typeof excludeFolders !== 'object') {
    excludeFolders = [excludeFolders];
  }

  console.log('RENDER in filetree.js', filters, excludeFolders);
  // Walker options
        ...
        if (comment_starters.indexOf(lines[i][0]) > -1) {
          ...
          if (!_.isUndefined(first_char)) {
            if (first_char.match(letters)) {
              ...
              //  + " / " + lines.length + " lines"
              output +=
                indent_file_comment(file, spaces_added) + formatted_comment;
...
function iterate_asset_folders(arr, callback) {
  render_code_folder_structure(
    arr[asset_type_counter].path,
    arr[asset_type_counter].title,
    arr[asset_type_counter].filters,
    function() {
      asset_type_counter++;
